...
**Phase 1 complete → Phase 2/3 next.** See `docs/ROADMAP.md` for the full arc.

Next concrete task: **stand up the SPRT harness before adding more search features.** Every Elo figure you read in an article or another engine's release notes is order- and engine-dependent. Only your own SPRT numbers mean anything.
...
1. `(768 → 256)x2 → 1` on ~100M positions — validates the pipeline, expect +300–400 Elo
2. `(768 → 1024)x2 → 1` with 8 output buckets on ~1B positions — this is where 3200 happens
3. King input buckets (4→8→16, horizontally mirrored) + Finny tables
4. L2/L3 layers, threat inputs, AVX-512/VNNI dispatch
